...
두 번쨰 경우는 반대로, 찾고자 하는 값이 중앙값보다 큰 경우이다. 이런 경우에는 중앙값과 중앙값 이전에 나열되어 있는 값들은 모두 우리가 찾고자 하는 값보다 작은 값이 된다. 마찬가지로, 해당 값들도 비교할 필요가 없기 떄문에 탐색 범위에서 제거해야 한다. 아래 코드를 통해 중앙값과 그 이전에 나열되어 있는 값들을 탐색 범위에서 제거할 수 있다.<br>
...
*어? 시작 인덱스와 끝 인덱스의 갑시 같아지게 되네?* <br>
