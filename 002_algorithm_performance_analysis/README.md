...
```
먼저, 이진탐색의 대상이 되는 배열 **`ex_arr`** 의 시작 인덱스는 **`first`** 라는 int 형 변수에, 끝 인덱스는 **`last`** 라는 int 형 변수에 저장한다. 그리고, 이 둘을 더한 뒤, 2 로 나눠서 구할 수 있는 중앙값의 위치(인덱스)는 int 형 변수 **`mid`** 에 저장한다.<br>

그 다음, 찾고자 하는 값 **`target`** 이 중앙값인 **`ex_arr[mid]`** 와 같다면 중앙값의 위치인 **`mid`**를 리턴하고 함수를 종료한다.<br>
그리고 만약, 찾고자 하는 값 **`target`** 이 중앙값인 **`ex_arr[mid]`** 와 다르다면, **`target`** 이 중앙값 보다 큰 경우에는 
