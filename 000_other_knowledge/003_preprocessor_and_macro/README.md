...
마지막으로는, 이렇게 매크로 함수를 사용하는 이유를 알아보고자, 해당 매크로 함수 사용 시 발행하는 장/단점을 정리해 보도록 하겠다.<br>
...
- #### 매크로 함수 사용 시 장점
  ...
  **`호출된 함수를 위한 Stack메모리 할당`** -> **`실행 위치의 이동과 해당 함수의 매개변수(파라미터)로 인자 전달`** -> **`return`**
   ...
   *[macro_function.c 의 실행결과]*
   ...
   해당 코드의 실행결과와 매크로 함수 호출부분, `SQUARE(2.5)`, `SQUARE(-5)`, `SQUARE(num)` 등을 보면, 전달인자의 자료형에 상관없이<br>
   정상적으로 치환되는 것을 알 수 있다.<br> 
   이것이 가능한 이유는 , 메크로 함수의 호출부분이 단순히 해당 매크로 함수의 몸체로 치환되기 때문에 가능하다.<br> 
