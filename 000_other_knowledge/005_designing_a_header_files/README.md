...
- #### 절대경로
  절대경로는 다음과 같이 헤더파일 이름 뿐만아니라, 드리이브 명과 디렉터리 경로를 포함하는 모든 경로를 명시하는 방법이다.<br>
...
- #### 상대경로 
  ...
  그리고, 이 문장을 포함하고 있는 소스파일이 `C:\AAA\BBB`에 저장되어 있다면, 이 문장의 헤더파일 검색경로도 `C:\AAA\BBB`가 된다.<br>
  (두 번째 방법을 사용하면, 해당 문장을 포함하고 있는 소스파일이 저장되어 있는 디렉터리에서 헤더파일을 탐색함)<br>

  또, 상위 디렉터리 `..`을 활용한 상대경로 지정도 가능하다.<br>
