...
- #### 원반이 3개 있는 경우
  ...
  그런데, 이 2개의 원반을 기둥 `B`로 옮기는 과정에서도 이전과 같은 문제가 발생한다. 2개의 원반을 기둥 `B`로 옮기기 위해선, 
  **2개 중 가장 큰 원반을 가장 먼저 기둥`B`에 옮겨 놓는것이 우선이 되어야 한다.**<br>
  그러기 위해서는, **2개 중 가장 큰 원반 위에 있는 1개의 원반을 기둥 `C`로 옮긴 다음** 2개 중 가장 큰 원반을 먼저<br>
  기둥`B`로 옮기고 나머지 원반을 기둥 `B`로 옮겨야 한다.<br>
  
  즉, 3개의 원반을 옮기기 위해서는 2개의 원반을 옮기는 문제로 세분화 되고, 2개의 원반을 옮기기 위해서는 1개의 원반을 옮기는 문제로 세분화 된다.<br>
  결국 결론적으로, 원반 3개를 옮기는 문제는 원반 1개를 옮기는 문제로 세분화가 된다.<br>
  
  이러한 과정을 하나의 규칙으로 표현해 보겠다.<br>
...
이렇게 나온 규칙을 일반화 해서 n개의 원반을 기둥`C`로 옮기는 과정을 정리해 보면 다음과 같다.<br>
> 1. 작은원반 n-1 개를 기둥 `A` 에서 `B`로 이동
> 2. 큰 원반 1개를 기둥 `A`에서 `C`로 이동
> 3. 작은원반 n-1 개를 기둥 `B`에서 `C`로 이동

결국 `A`에 꽂혀있는 원반 n개를 기둥 `C`로 옮기는 과정은 다음의 규칙이 재귀적으로 반복되면서 이루어지게 된다. 
