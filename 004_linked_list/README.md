...
- #### 노드를 자유 저장소(힙 영역)에 생성하게 되는 경우
  ...
  이때, `malloc()` 함수가 인자로 전달받게 되는 정수는 0과 양의 정수만 가능한데, `sizeof()`함수의 반환형이 `unsigned int`형 이기 때문에<br>
  `Node`의 크기에 해당하는 양의 정수인 `sizeof(Node)`를 인자로 전달한다.<br>
  
  그러면, `Node`의 크기에 해당하는 바이트 크기의 메모리 공간이 heap영역에 할당되게 된다.<br>
  
  그 후,`malloc()` 함수는 `Node`의 크기만큼 heap영역에 할당된 메모리 공간을 이루는 첫 번째 바이트의 주소값을 포인터 변수 `NewNode`에 반환한다.<br>
  <br>
  
  이제, `Node` 형으로 해당 함수를 형변환 하는 과정을 살펴보자.<br>
  ...
  때문에 `malloc()` 함수는 일단, 어떤 타입이든 상관 없이 주소값을 담을 수 있는 `void 형 포인터` 형태로 주소값을 반환한다.<br>
  ...
  이 상태에서 heap영역에 새로 할당한 메모리 공간에 접근하기 위해서는, 
  
  때문에, 프로그래머는 기본적으로 `void`형 으로 반환되는 이 주소값을 원하는 type으로 형 변환하여 할당된 메모리 공간으로 접근이 가능할 수 있게 해야 한다.<br>
