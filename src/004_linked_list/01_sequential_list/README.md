...
이러한 ADT는 해당 자료구조를 구현할 프로그래밍 언어에 종속적으로 정의가 가능한데, 이러한 경우, 해당 언어마다 내용이 달라지기도 한다.<br>
C 라는 언어는 내가 변수를 선언한다고 해서, 자동으로 초기화 해주는 함수가 호출되지 않는다. 때문에, 초기화의 과정을 별도로 명령해야 <br>
한다.<br>
