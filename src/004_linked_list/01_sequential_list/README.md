...
### 4) `arr`에 저장되어 있는 데이터의 수와 데이터의 출력
...
- #### 구조체 데이터의 삭제 : 배열 기반 리스트에서의 삭제 & Heap영역에 할당된 구조체 메모리까지 삭제 
  ...
  > 해당 프로그램에서 리스트에 저장한 데이터는 '`Point`구조체 변수의 주소값' 이다. 물론, 이 주소값은 `Point`구조체를<br>
    `malloc`함수를 통해 동적으로 할당한 결과이기 때문에, 반드시 `free`함수를 통한 메모리의 해제과정을 거쳐야 한다!<br><br>
    `LRemove`함수처럼 데이터를 소멸하는 함수는 소멸된 데이터를 반환하도록 정의해야 한다.<br>
    메모리 소멸의 기회를 줄 수 있어야 하는 것이다.
<br>
