...
>   해주는 `LNext` 함수. 이 두개의 함수를 통해 구현이 가능하다.<br><br>
>   이렇게 첫 번째 데이터를 탐색하여 초기화 해주는 기능과, 첫 번째 데이터 이후의 데이터를 탐색하여 초기화 해주는 기능으로<br>
>   나누어서, 저장된 데이터의 탐색 및 탐색 초기화 기능을 구현하는 이유는 다음과 같다.<br><br>
>   순차적으로 나열되어 저장되어 있는 데이터를 탐색하는데 있어, 다시 처음부터(첫 번째 데이터 부터) 탐색을 하고 싶은 경우<br>
>   그 처음 이라는 기준점이 되는 것이 바로, 첫 번째 데이터를 반환하는 아래의 `LFirst` 함수이다.<br><br>
