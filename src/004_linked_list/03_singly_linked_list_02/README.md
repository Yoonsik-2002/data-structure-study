...
##### `SInsert`함수의 이해 1 : `pred`를 활용한 새 노드의 삽입 과정, `pred`는 왜 dummy node부터 가리켜야 하는지에 대한 이유
`SInsert`함수는 정렬 기준이 리스트에 등록되어 있는 경우(`plist -> comp != NULL`), 해당 정렬 기준을 토대로 데이터를 삽입하여 정렬시키는 함수이다.<br>

먼저, `SInsert`함수의 작동과정을 그림과 함께 이해해 보도록 하겠다. 예를 들어, 아래와 같이 오름차순으로 정렬되어 있는 연결 리스트가 존재한다고 해보자. 해당 연결 리스트의 이름은 `slist`이다.<br>
<br>
...
이렇게 새 노드 추가가 완료되면, 아래 코드를 통해 리스트에 저장되어 있는 노드의 개수를 하나 늘려준다.<br>
```c
(plist -> numOfData)++;
```
<br>

##### `SInsert`함수의 이해 2: 정렬의 핵심인 while 반복문
자, 이제 `SInsert`함수 에서의 새 노드가 삽입 되어야 할 위치를 찾는 반복문을 작성해 보도록 하겠다. 해당 반복문을 통해, 새 노드가 들어가야할 위치 이전에 존재하는 노드를 탐색하여 `pred`가 가리키게 하고, 이를 통해 `pred`가 가리키는 노드 기준 오른쪽에 새 노드를 추가한다.<br>
<br>

해당 while반복문의 조건과 내용은 아래와 같다.<br>
```c
while(pred -> next != NULL && plist -> comp(data, pred -> next -> data) != 0) {
  pred = pred -> next; // 새 노드가 들어갈 위치 바로 이전에 존재하는 노드를 탐색하여 pred가 가리키게 함(comp가 0을 반환하였다는 것은 그 자리를 탐색 완료 하였다는 뜻. 때문에, 반복 종료)
}
```
<br>

이 조건문을 나눠서 해석해보면 다음과 같다.<br>
- ##### 반복 조건1) `pred -> next != NULL`
  `pred`가 가리키고 있는 노드가 리스트의 마지막 노드인지 아닌지? (`pred`가 가리키고 있는 노드가 리스트의 마지막 노드라면 반복 종료)<br>

- ##### 반복 조건2) `plist -> comp(data, pred -> next -> data) != 0`
  새 노드에 들어가는 데이터와 `pred`의 다음 노드의 데이터의 정렬 우선순위 비교(누가 더 head에 가깝게 위치해야 하나?)를 위한 함수를 호출한다. 새 노드의 데이터와 `pred`의 다음 노드의 데이터를 비교하는 이유는 `pred`는 dummy node부터 가리키기 때문이다.<br>

  `comp`함수가 0을 반환하였다는 것은 새 노드가 들어갈 자리를 찾았다는 뜻이며, 해당 경우, `pred`가 새 노드가 들어가야 할 자리 이전에 존재하는 노드를 가리키는 상태로 while문이 종료되게 된다. 새 노드는 `pred`가 그 시점에 가리키고 있는 노드 기준 오른쪽에 추가된다. <br>
<br>

결국 해당 while문의 동작은 다음과 같이 정리하여 설명하는 것이 가능하다.<br>

*"`pred`가 가리키고 있는 노드가 연결 리스트의 마지막 노드가 아니고, 새 노드가 들어가야 하는 위치도 탐색하지 못한 경우, `pred`를 다음 노드로 이동 시킨다."*<br>
<br>

##### 정렬의 기준을 설정하기 위한 함수의 정의 
마지막으로 `comp`에 전달되어 연결 리스트의 정렬 기준이 되어주는 함수를 한번 작성해 보도록 하겠다. 전에도 설명하였듯이, 해당 함수가 반환하는 값, `0`과 `1`이 의미하는 것은 아래 기준을 바탕으로 프로그래머가 마음대로 정하는 것이 가능하다.<br>
 - 두 개의 인자를 전달받도록 함수를 정의한다.<br>
 - 두 인자 중, 첫 번째 인자의 정렬 우선순위가 높은 경우, 0을 반환하고, 두 번째 인자의 정렬 우선순위가 높은 경우 1을 반환한다.<br>
<br>

##### 오름차순으로 연결리스트의 노드가 정렬되도록 하는 함수
위 조건을 바탕으로 먼저 오름차순으로 노드를 정렬하게 하는 함수를 작성해 보도록 하겠다. 해당 함수의 이름은 `WhoIsPrecede`이다. 먼저, 조건에 맞도록 두 개의 인자를 전달 받도록 함수를 정의해 보았다.<br>
```c
WhoIsPrecede (int d1, int d2) {
 // ...
}
```
<br>

while문에서 `comp`함수가 호출되는 형식을 보면, 새 노드에 들어가는 데이터는 매개변수 `d1`에, `pred`의 다음 노드가 저장하고 있는 데이터는 매배견수 `d2`에 전달되어 호출되게 된다.<br>

우리가 while문을 반복하여 `comp`함수를 호출하여 구하고자 하는 것은 `d1`에 해당하는 데이터를 가진 새 노드의 자리를 찾기 위함이다. 오름차순의 경우, 두 노드가 가지는 데이터의 값을 비교해 보았을 때 더 작은 데이터 값을 가진 노드가 head에 더 가깝게 위치하도록 정렬 우선순위를 가지게 된다.<br>
<br>

이를 바탕으로 `WhoIsPrecede`함수를 완성해 보면 다음과 같다.<br>
```c
WhoIsPrecede(int d1, int d2) {
  if(d1 < d2)
    return 0;
...
반대로, 내림차순의 경우에는 다음과 같이 구현이 가능할 것이다.<br>
```c
WhoIsPrecede(int d1, int d2) {
  if(d1 > d2)
    return 0;
