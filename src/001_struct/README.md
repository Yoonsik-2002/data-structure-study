...
## 구조체 배열
구조체 배열이란, 연속적인 메모리 공간에 여러개의 구조체를 저장하는 데이터 구조이다.<br>
각 구조체는 동일한 구조를 가지지만, 서도 다른 데이터를 저장할 수 있다.<br>

구조체 배열을 선언할 때, 컴파일러는 각 구조체의 크기를 고려하여 연속된 메모리 공간을 할당한다.<br>
자, 그렇다면 구조체 배열의 각 요소에는 무엇이 저장될까?<br>
<br>

구조체 배열의 각 요소에는 기본적으로, 구조체의 값만이 저장된다. 이러한 구조체 배열의 각 요소는 해당 구조체의 인스턴스를<br>
나타내며, 구조체의 멤버변수들의 값이 저장된다.<br>
