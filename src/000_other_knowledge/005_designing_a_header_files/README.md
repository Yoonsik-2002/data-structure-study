...
헤더파일의 중복삽입 자체는 문제가 되지 않는다. 아래와 같이, 우리가 흔히 헤더파일에 삽입하는 외부파일의 변수나 함수의 선언과 같은<br> 유형의 내용들은 두번 이상 삽입이 되어도 문제가 되지 않는다.<br> 
...
왜냐하면, 이는 컴파일러에게 전달하는 메세지에 지나지 않기 때문이다.<br>

이러한 메세지를 두 번 이상 전달한다고 해서, 문제가 되지는 않는다.<br>
그리고, 이러한 유형의 선언들은 실행파일의 크기와도 상관이 없다.<br>

이유는, 이러한 선언들은 여러 번 삽입된다고 해서 컴파일러에 의한 컴파일 과정이 모두 실행되고 나서 생성되는 실행파일의 내용에<br>
직접적인 영향을 주는 내용이 아닐 뿐더러, 단지 컴파일러가 컴파일을 할 수 있도록 도움을 줄 뿐이므로, 해당 유형의 내용을 담은 <br>
헤더파일의 중복된 삽입 자체는 문제가 되지 않는다고 할 수 있다.<br>
(단순히, 동일한 변수의 선언과 함수의 선언의 횟수는 실행파일의 내용에 있어서 큰 영향을 주지는 않는다. 해당 변수나 함수를<br>
사용하느냐 아니냐가 중요함. 선언은 컴파일러에게 '이러한 변수와 함수를 사용할 수 있어요' 라는 정보를 전달해주는 메세지일 뿐이다.)<br>

그럼, 실행파일의 내용에 직접 영향을 주는 내용을 무엇이 있을까?

바로, 위 예시코드에서 두 번 중복하여 삽입된 '구조체의 정의'가 있다. 구조체의 정의는 단순 변수나 함수의 선언과는 달리,<br>
실행파일의 내용에 직접적인 연관이 있는 내용이다.<br>
**구조체를 어떻게 정의하느냐에 따라서, 해당 실행파일의 크기 뿐만 아니라, 내용까지 달라지기 때문이다.** <br>
따라서, 이러한 형태의 정의는 두 번 이상 중복될 수 없다.<br>

> ##### 헤더파일의 중복삽입 문제 - <br>
> 함수나 변수의 선언과 같은 단순히 '컴파일러에게 이러이러한 함수나 변수의 사용이 가능합니다' 라는 메세지를 전달해주는 내용은<br>
> (헤더파일을 통해) 아무리 중복되어 소스코드 내에 삽입이 되어도 실행파일의 내용이나 크기에는 영향을 미치지 않기 떄문에 문제가<br>
  되지 않는다.<br>
...
위에서 설명하였듯이, 구조체의 선언 및 정의의 경우, 실행파일의 내용과 크기에 영향을 미치기 때문에, 헤더파일의 중복 삽입으로 인해<br>
같은 내용의 구조체 선언 및 정의가 소스파일에 여러번 삽입될 시, 컴파일 에러를 발생시킨다.<br>
